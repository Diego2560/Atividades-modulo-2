...
function excluirInstrutores(req, res) {
    const instrutor = listaDeInstrutores.find(
        (instrutor) => instrutor.id === Number (req.params.idConsultado)
   );

    const indece = listaDeInstrutores.indexOf(instrutor);
    listaDeInstrutores.splice(indece, 1);
    res.json(instrutor);
...
module.exports = { consultarTodosOsInstrutores,
    consultarUmInstrutor, criarInstrutor,
    editarInstrutor, substituirInstrutor,
    excluirInstrutores };